...
from pyspark.sql.functions import rank, col, to_timestamp
...
def filter_data(df):
  ...
  terms = ['coronavirus','covid','epidemic','pandemic','quarantine','respiratory','sars','vaccine','virus']
  df = df.filter(df.source_url.rlike('|'.join(terms)))
  df = df.filter(df.date >= 20190101)
